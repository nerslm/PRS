...
Before starting, check if `PROJECT.md` exists in the project root.
If found:
- Read the `## Config` section and use its values as defaults for all parameters (venue, auto_proceed, human_checkpoint, etc.). Inline arguments (— key: value) override PROJECT.md values.
- Read the `## Progress` section to understand what has already been done. If a stage is already marked done and its output files exist, skip to the next incomplete stage.
After each stage completes, append one summary line to the `## Progress` section: `- [date] [stage] done → [output files] | [key decision or result]`
