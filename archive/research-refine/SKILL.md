...
- **VENUE = `NeurIPS`** — Target venue for reviewer framing. Override: `/research-refine "topic" — venue: nature-communications`. Determines the reviewer persona and evaluation criteria.
...
**Before composing the prompt**, read `/root/workspace/research/.claude/skills/shared-references/venues/{VENUE_SLUG}.md` and use its **Refine Persona** section as the reviewer identity. If the file does not exist, fall back to `/root/workspace/research/.claude/skills/shared-references/venues/neurips.md`. Then send the full proposal to GPT-5.4 for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.
...
mcp__codex__codex:
  ...
  prompt: |
    [INSERT the Refine Persona from the venue profile file here]
    ...
    7. **Venue Readiness**: If executed well, would the contribution feel sharp and timely enough for [VENUE]?
