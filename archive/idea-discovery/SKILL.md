...
Before starting, check if `PROJECT.md` exists in the project root. If found, read the `## Config` section and use its values as defaults (venue, auto_proceed, etc.). Inline arguments override PROJECT.md. After this stage completes, append one summary line to `## Progress`.
...
- **VENUE = `NeurIPS`** — Target venue for reviewer framing. Override: `/idea-discovery "topic" — venue: nature-communications`. Passed through to `/research-review` and `/research-refine-pipeline`.
...
**What this does:**
- GPT-5.4 xhigh acts as a senior reviewer for [VENUE] (using the venue profile from `/root/workspace/research/.claude/skills/shared-references/venues/{VENUE_SLUG}.md`)
