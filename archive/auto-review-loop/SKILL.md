...
Before starting, check if `PROJECT.md` exists in the project root. If found, read the `## Config` section and use its values as defaults (venue, human_checkpoint, etc.). Inline arguments override PROJECT.md. After each round completes, append one summary line to `## Progress`.
...
- **VENUE = `NeurIPS`** — Target venue for reviewer framing. Override: `/auto-review-loop "topic" — venue: nature-communications`. Determines the reviewer persona and evaluation criteria.
- **VENUE_SLUG** = VENUE in lowercase (e.g., `NeurIPS` → `neurips`, `PRL` → `prl`). The slug must match a filename in `/root/workspace/research/.claude/skills/shared-references/venues/` (relative to project root).
