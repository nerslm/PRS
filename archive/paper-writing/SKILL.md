...
Before starting, check if `PROJECT.md` exists in the project root. If found, read the `## Config` section and use its values as defaults (venue, human_checkpoint, etc.). Inline arguments override PROJECT.md. After this stage completes, append one summary line to `## Progress`.
...
- **VENUE = `PRL`** — Target venue. Options: `ICLR`, `NeurIPS`, `ICML`. Affects style file, page limit, citation format.
